...
def centroid_main(text, coord_format_id=0, perctl=65, zoom_start=14):
    def form_text_res(s, new_line):
        print(new_line)
        return '\n'.join([s, new_line])

    pnts = parse_text(text, coord_format_id)
    t_res = f'Кількість точок: {len(pnts):d}'
    ...
    t_res = form_text_res(t_res, 
        f'Центроїд:\n{cntr.lat:.6f}, {cntr.lon:.6f}')
    t_res = form_text_res(t_res, '№, коорд, відстань (м):')
    for i,_ in enumerate(pnts):
        t_res = form_text_res(t_res, 
          f'{i+1:00d}, {pnts[i][0]:.6f}, {pnts[i][1]:.6f}, {dist_l[i]:.1f}')

    dist = dist_stats(dist_l, perctl)
    t_res = form_text_res(t_res, 
        f'Середня відстань (м) - {dist["mean"]:.1f}')
    t_res = form_text_res(t_res, 
