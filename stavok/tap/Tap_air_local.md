...
```
    ...
    MitoWidget(analysis_data_json='{"analysisName": "UUID-a06e0695-d46c-4ace-949d-c5b3570a9228", "code": {"imports…
...
factor = factors[0] # Later this will substitute by UI
```
...
```python
#hv.extension('bokeh') ## uncomment this line at Colab
...
To launch the web-app as a server, please type the following in the command prompt:

```
> panel serve Tap_air_local.py
```

Here `Tap_air_local.py` is the name of our Python code embracing all the actions above. Since *Panel* works similarly with Jupyter notebooks you can equally run this command as follows: `panel serve Tap_air_local.ipynb`.

After that you'll be asked to go to the browser and launch the app from the web-address bar:

    http://localhost:5006/Tap_air_local

#### Command script

We can consolidate these actions in a single command script. Let's call it `Tap_air_local.bat` and put near the main code file `Tap_air_local.py`. For Windows 10 this script will have the following look:
...
- `<user_name>` - your user name in Windows 10 at your local computer;
- `<env_name>` - Python Anaconda environment name of your web app;
- Tap_air_local_Link.url - this label file for Windows, containing the local web-address to launch the app: http://localhost:5006/Tap_air_local.

> **Warning!** While creating the label file in Windows, name it without extension `.url` - just `Tap_air_local`. The extension is really hidden and is added to the file automatically.

After that you just need to run in the prompt or double click it in the File Explorer:

```
> Tap_air_local
```

You can easily modify this command script for other operation system.